...
__all__ = [
    'perform_lpa_star', 'perform d_star_lite', 'perform_a_star'
]
...
def update_node_lpa(
        node_to_update, end, g_dict, rhs_dict, open_set
):
    global COUNT
    global PERCOLATES
    global ACCESSES
    ...
    while (
        ...
        if g_dict[current] > rhs_dict[current]:
            ...
            if (
                ...
                and not current.is_invisible_barrier()
            ):
                current.make_closed()
...
# main method for LPA*
def perform_lpa_star(
        draw_func, grid, start, end, invisible_barriers
):
    global PERCOLATES
    global EXPANSIONS
    global ACCESSES
...
def update_node_d_lite(
    ...
    if locally_inconsistent and index_of_item is not None:
        ...
        if not (
            node_to_update.is_invisible_barrier() 
    ...
    while (
        ...
        if k_old < k_new:
            ...
            if g_dict[current] > rhs_dict[current]:
                ...
                if not (
                    current is end 
                    or current.is_invisible_barrier() 
                    or current.is_path()
                ):
                    current.make_closed()
...
def a_star_compute_shortest_path(draw_func, start, end, g_dict, f_dict, open_set):
    global PERCOLATES
    global EXPANSIONS
    global ACCESSES
    global COUNT
...
def perform_a_star(
        draw_func, grid, start, end, invisible_barriers, travel
):
    global PERCOLATES
    global EXPANSIONS
    global ACCESSES
    ...
    if travel:
        a_star_with_travel(
            draw_func, start, end, grid, g_dict, f_dict
        )
...
def a_star_with_travel(draw_func, start, end, grid, g_dict, f_dict):
    global PERCOLATES
    global ACCESSES
    global COUNT
        ...
        while start is not end:
            ...
            if nodes_changed:
                ...
                if not a_star_compute_shortest_path(
                    draw_func, end, start, g_dict, f_dict, 
                    open_set_heap
                ):
                    # leave while loop if a path to start from goal 
                    # does not exist
